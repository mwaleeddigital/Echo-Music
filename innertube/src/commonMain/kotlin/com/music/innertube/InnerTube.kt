...
import com.music.innertube.utils.getDefaultLocale
import com.music.innertube.utils.sha1
import com.music.innertube.utils.currentTimeMillis
...
class InnerTube {
  ...
  var locale = getDefaultLocale().let { (gl, hl) ->
    YouTubeLocale(gl = gl, hl = hl)
  }
  ...
  private fun createClient() =
    HttpClient {
      ...
      // Platform-specific engine configs (like OkHttp connection pools, caches, etc.)
      // are omitted here to maintain pure Kotlin compatibility across Web/Desktop.
      // Ktor will automatically select the best available engine.
      engine {
      ...
      defaultRequest {
        val targetUrl = corsProxyUrl?.let { proxy ->
          if (proxy.endsWith("/")) "$proxy${YouTubeClient.API_URL_YOUTUBE_MUSIC}"
          else "$proxy/${YouTubeClient.API_URL_YOUTUBE_MUSIC}"
        } ?: YouTubeClient.API_URL_YOUTUBE_MUSIC
        url(targetUrl)
  ...
  /**
   * Simple retry wrapper for transient IO errors (socket aborts, timeouts). Retries the given block
   * up to [maxAttempts] times with exponential backoff. Cancellation is respected since [delay]
   * will throw if the coroutine is cancelled.
   */
  ...
  suspend fun search(
    ...
    httpClient.post("search") {
      ...
      setBody(
        SearchBody(
          context = client.toContext(locale, visitorData, if (effectiveLogin) dataSyncId else null),
          query = query,
          params = params
        )
      )
    ...
    httpClient.post("player") {
      ...
      setBody(
        PlayerBody(
          ...
          context =
            client.toContext(locale, visitorData, dataSyncId).let {
  ...
  suspend fun feedback(client: YouTubeClient, tokens: List<String>) = withRetry {
  ...
  suspend fun getUploadCustomThumbnailLink(client: YouTubeClient, contentLength: Int) = withRetry {
  ...
  suspend fun removeThumbnailPlaylist(client: YouTubeClient, playlistId: String) = withRetry {
    httpClient.post("browse/edit_playlist") {
      ytClient(client, setLogin = true)
      setBody(
        EditPlaylistBody(
  ...
  /**
   * Initialize a song upload to YouTube Music. Returns the upload URL in the X-Goog-Upload-URL
   * header.
   */
  suspend fun initSongUpload(filename: String, contentLength: Long) = withRetry {
    val authUser = "0"
  ...
  suspend fun uploadSongData(
    ...
    httpClient.post(uploadUrl) {
      ...
      onUpload { bytesSentTotal, contentLength ->
        contentLength?.let { onProgress?.invoke(bytesSentTotal.toFloat() / it.toFloat()) }
      }
    }
  }
  ...
  suspend fun getMediaInfo(videoId: String): Result<MediaInfo> = runCatching {
    val response =
      next(client = YouTubeClient.WEB, videoId, null, null, null, null, null).body<NextResponse>()
    ...
    return@runCatching MediaInfo(
      ...
      authorId =
        baseForInfo?.owner?.videoOwnerRenderer?.navigationEndpoint?.browseEndpoint?.browseId,
      authorThumbnail =
        baseForInfo
          ?.owner
          ?.videoOwnerRenderer
