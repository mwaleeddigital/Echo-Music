...
data class AlbumPage(
  ...
  companion object {
    ...
    fun getYear(response: BrowseResponse): Int? {
      val title =
        getHeader(response)?.subtitle ?: response.header?.musicDetailHeaderRenderer?.subtitle
    ...
    fun getSongs(response: BrowseResponse, album: AlbumItem): List<SongItem> {
      ...
      val songs = shelfRenderer?.contents?.getItems()?.mapNotNull { getSong(it, album) }
    ...
    fun getSong(renderer: MusicResponsiveListItemRenderer, album: AlbumItem? = null): SongItem? {
      ...
      return SongItem(
        ...
        album =
          album?.let { Album(it.title, it.browseId) }
            ?: renderer.flexColumns
              .getOrNull(2)
              ?.musicResponsiveListItemFlexColumnRenderer
              ?.text
        ...
        thumbnail =
          renderer.thumbnail?.musicThumbnailRenderer?.getThumbnailUrl() ?: album?.thumbnail!!,
        explicit =
