...
object PageHelper {
  ...
  /**
   * Check if an icon type is a library-related icon (for filtering menu items) Excludes
   * KEEP/KEEP_OFF which are for "Pin to Listen Again"
   */
   ...
   *
   * This function iterates through ALL toggle menu items and extracts tokens based on their icon
   * types, ensuring we don't confuse library tokens with "Pin to Listen Again" tokens
   * (KEEP/KEEP_OFF).
   *
   ...
   */
  fun extractFeedbackToken(
    menu: Menu.MenuRenderer.Item.ToggleMenuServiceRenderer?,
    type: String
  ): String? {
